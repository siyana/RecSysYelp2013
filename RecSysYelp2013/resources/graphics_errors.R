...
library(ggplot2)
p <- ggplot() + 
  geom_line(data = errors, aes(y = svdPref, x = 1:9, color = "red")) +
  geom_line(data = errors, aes(y = userPref, x = 1:9, color = "blue"))  +
  geom_line(data = errors, aes(y = itemPref, x = 1:9, color = "green")) +
  geom_line(data = errors, aes(y = svdPlusPlusPref, x = 1:9, color = "yellow")) +
...
lambda = c(0.025,0.05,0.05,0.05,0.05,0.1,0.1, 0.1,0.1)
...
numIterations = c(10,10,10,10,20,20,20, 30,30)
...
attach(errorsSVD)
numIterationsSVD = c(50,100,100,100,100,100,100, 100,150)
...
attach(errorsSVDPl)
numFeaturesSVDPl = c(2,2,2,2,2,2,2,3,3)
